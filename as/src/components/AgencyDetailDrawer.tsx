  ...
  return (
    <div className="fixed inset-0 z-50 overflow-hidden bg-slate-900/40 backdrop-blur-xs flex justify-end animate-in fade-in duration-200">
      <div className="w-full max-w-xl bg-white h-full shadow-2xl flex flex-col justify-between border-l border-slate-200 animate-in slide-in-from-right duration-200">
        {/* Drawer Header */}
        <div className="p-6 border-b border-slate-200 bg-slate-50/50 relative">
          <button
            onClick={onClose}
            className="absolute top-5 right-5 p-2 rounded-full hover:bg-slate-200 text-slate-500 transition-colors cursor-pointer"
            title="Kapat"
          >
            <X className="w-5 h-5" />
          </button>
          ...
          <div className="flex items-start space-x-4">
            <div className="w-16 h-16 rounded-2xl bg-white border border-slate-200 p-2 shadow-sm flex items-center justify-center shrink-0">
              <img
                src={agency.logo}
                alt={agency.adi}
            ...
            <div className="flex-1 pr-6">
              <div className="flex items-center space-x-3">
                <h2 className="text-xl font-bold text-slate-900 font-sans">{agency.adi}</h2>
              </div>

              {/* Relationship Score Badge */}
              <div className="mt-2 flex items-center space-x-3">
                <div className="flex items-center space-x-1.5 bg-blue-50 border border-blue-200 text-[#0057B8] px-3 py-1 rounded-full text-xs font-bold shadow-2xs">
                  <span>İlişki Skoru</span>
                  <span className="w-1.5 h-1.5 rounded-full bg-[#0057B8]"></span>
                  <span className="text-sm font-extrabold text-[#0057B8]">{agency.iliskiSkoru}</span>
                </div>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 mt-5">
            <div className="bg-white p-2.5 rounded-xl border border-slate-200 text-center shadow-2xs">
              <p className="text-[11px] text-slate-500 font-medium">Konum</p>
              <p className="text-xs font-bold text-slate-900 mt-0.5 truncate">📍 {agency.il}</p>
            </div>
            <div className="bg-white p-2.5 rounded-xl border border-slate-200 text-center shadow-2xs">
              <p className="text-[11px] text-slate-500 font-medium">Şube</p>
              <p className="text-xs font-bold text-slate-900 mt-0.5 truncate">🏢 {agency.subeSayisi} Şube</p>
            </div>
            <div className="bg-white p-2.5 rounded-xl border border-slate-200 text-center shadow-2xs">
              <p className="text-[11px] text-slate-500 font-medium">Personel</p>
              <p className="text-xs font-bold text-slate-900 mt-0.5 truncate">👥 {agency.personelSayisi} Personel</p>
            </div>
            <div className="bg-white p-2.5 rounded-xl border border-slate-200 text-center shadow-2xs">
              <p className="text-[11px] text-slate-500 font-medium">Kıdem</p>
              <p className="text-xs font-bold text-slate-900 mt-0.5 truncate">📅 {agency.katilimYili}'ten beri</p>
            </div>
          </div>

          {/* Drawer Navigation Tabs */}
          <div className="flex border-b border-slate-200 mt-6 -mb-6 space-x-6">
            <button
