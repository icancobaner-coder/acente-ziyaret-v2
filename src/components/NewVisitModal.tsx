...
export const NewVisitModal: React.FC<NewVisitModalProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-900/50 backdrop-blur-xs flex items-center justify-center p-2 sm:p-6 animate-in fade-in duration-200">
      <div className="bg-white rounded-2xl max-w-3xl w-full shadow-2xl border border-slate-200 overflow-hidden my-2 sm:my-8 animate-in zoom-in-95 duration-200 max-h-[95vh] flex flex-col">
        {/* Modal Header */}
        <div className="p-4 sm:p-6 border-b border-slate-200 bg-slate-50 flex items-center justify-between shrink-0">
          <div className="flex items-center space-x-2.5 sm:space-x-3">
            <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-[#0057B8] text-white flex items-center justify-center font-bold shadow-2xs shrink-0">
              <Sparkles className="w-5 h-5" />
            </div>
            <div>
              <h2 className="text-base sm:text-lg font-bold text-slate-900">Yeni Ziyaret Kaydı</h2>
              <p className="text-[11px] sm:text-xs text-slate-500">Saha veya online acente görüşmesini kaydedin</p>
            </div>
          </div>

          <button
            onClick={onClose}
            className="p-2 rounded-full hover:bg-slate-200 text-slate-500 transition-colors cursor-pointer min-h-[44px] min-w-[44px] flex items-center justify-center"
            aria-label="Kapat"
          >
            <X className="w-5 h-5" />
          </button>
        ...
        <form onSubmit={handleSubmit} className="p-4 sm:p-6 space-y-4 sm:space-y-6 overflow-y-auto flex-1">
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 sm:gap-4">
            ...
            <div>
              ...
              <select
                ...
                className="w-full bg-slate-50 border border-slate-200 text-xs rounded-xl px-3 py-2.5 font-medium text-slate-900 outline-none focus:border-[#0057B8] min-h-[40px]"
              >
                {agencies.map((a) => (
          ...
          {/* Large Textarea for Ziyaret Notu */}
          <div>
            <div className="flex items-center justify-between mb-1.5 flex-wrap gap-1">
              ...
              <button
                ...
                className="text-xs font-bold text-[#0057B8] hover:underline flex items-center space-x-1 cursor-pointer min-h-[36px]"
              >
            ...
            <textarea
              ...
              className="w-full bg-slate-50 border border-slate-200 text-xs rounded-xl p-3 text-slate-900 outline-none focus:border-[#0057B8] font-sans leading-relaxed"
            />
          </div>

          {/* Blue AI Card */}
          <div className="bg-blue-50/80 border border-blue-200 p-3.5 sm:p-4 rounded-2xl flex items-start space-x-3">
          ...
          {analyzedMemory && (
            <div className="bg-slate-900 text-white p-3.5 sm:p-4 rounded-2xl space-y-3 animate-in fade-in duration-200">
          ...
          <div className="flex items-center justify-end space-x-3 pt-3 border-t border-slate-200 shrink-0">
            <button
              type="button"
              onClick={onClose}
              className="px-4 sm:px-5 py-2.5 rounded-xl text-xs font-bold text-slate-600 bg-slate-100 hover:bg-slate-200 transition-all cursor-pointer min-h-[44px]"
            >
              İptal
            </button>
            <button
              type="submit"
              className="px-4 sm:px-6 py-2.5 rounded-xl text-xs font-bold text-white bg-[#0057B8] hover:bg-[#004290] shadow-md shadow-blue-600/20 transition-all cursor-pointer flex items-center space-x-2 min-h-[44px]"
            >
