  ...
  return (
    <div className="space-y-4 sm:space-y-6 pb-12">
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        <div>
          <h2 className="text-lg sm:text-xl font-bold text-slate-900 tracking-tight">Ziyaretler</h2>
        ...
        <button
          ...
          className="flex items-center space-x-2 bg-[#0057B8] hover:bg-[#004290] text-white text-xs font-bold px-3.5 sm:px-4 py-2.5 rounded-xl shadow-md transition-all cursor-pointer min-h-[44px]"
        >
      ...
      <div className="bg-white p-3.5 sm:p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3">
        <div className="flex items-center space-x-2 text-xs font-bold text-slate-700 pb-2 border-b border-slate-100">
          <Filter className="w-4 h-4 text-[#0057B8] shrink-0" />
        ...
        <div className="grid grid-cols-2 xs:grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-3">
          {/* 1. Tarih Aralığı */}
          <div>
            <label className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block mb-1">Tarih Aralığı</label>
            <select
              value={dateRange}
              onChange={(e) => setDateRange(e.target.value)}
              className="w-full bg-slate-50 border border-slate-200 text-xs rounded-xl px-2.5 py-2 text-slate-800 outline-none focus:border-[#0057B8] min-h-[38px]"
            >
          ...
          {/* 2. İl */}
          <div>
            <label className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block mb-1">İl</label>
            <select
          ...
          {/* 3. Temsilci */}
          <div>
            <label className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block mb-1">Temsilci</label>
            <select
              value={selectedRep}
          ...
          {/* 4. Acente */}
          <div>
            <label className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block mb-1">Acente</label>
            <select
          ...
          {/* 5. Durum */}
          <div>
            <label className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block mb-1">Durum</label>
            <select
          ...
          {/* 6. Ziyaret Türü */}
          <div>
            <label className="text-[10px] sm:text-[11px] font-semibold text-slate-500 block mb-1">Ziyaret Türü</label>
            <select
              value={selectedType}
      ...
      <div className="block sm:hidden space-y-3">
        {filteredVisits.map((visit) => (
          <div
            ...
            className="bg-white p-4 rounded-2xl border border-slate-200 shadow-2xs space-y-3 active:scale-98 transition-all cursor-pointer"
          >
            <div className="flex items-start justify-between">
              <div className="flex items-center space-x-3">
                <div className="w-10 h-10 rounded-xl bg-slate-100 p-1 border border-slate-200 overflow-hidden shrink-0 flex items-center justify-center">
                  {visit.acenteLogo ? (
                    <img src={visit.acenteLogo} alt={visit.acenteAdi} className="w-full h-full object-cover rounded-lg" />
                  ) : (
                    <Building2 className="w-5 h-5 text-slate-400" />
                  )}
                </div>
                <div>
                  <h3 className="text-xs font-bold text-slate-900">{visit.acenteAdi}</h3>
                  <p className="text-[11px] text-slate-500 mt-0.5">📍 {visit.il} • {visit.temsilci}</p>
                </div>
              </div>
            ...
            <div className="flex items-center justify-between text-[11px] text-slate-600 bg-slate-50 p-2.5 rounded-xl">
              <span>📅 {visit.tarih} - 🕒 {visit.saat}</span>
            ...
            {visit.not && (
              <p className="text-xs text-slate-600 line-clamp-2 leading-relaxed bg-slate-50/50 p-2 rounded-lg italic">
                "{visit.not}"
              </p>
            )}
          </div>
      ...
      <div className="hidden sm:block bg-white rounded-2xl border border-slate-200 shadow-2xs overflow-hidden">
